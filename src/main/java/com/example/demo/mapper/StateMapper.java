...
public interface StateMapper {
    ...
    @Select("select * from info_device where tower_id = #{towerId}")
    Map<String, Object> getDevMapByTowerId(String towerId);

    @Select("select t1.name as LeftStationName,t2.name as RightStationName from info_station t1, info_station t2, info_line t3  where t1.id = t3.left_station and t2.id = t3.right_station and t3.id = #{LineId}")
    Map<String,Object> queryStationNameByLineId(String LineId);
...
//    		@Param("userID") String userID);
   ...
   //@Select("select * from info_device where (line_name = #{CommLine}) limit #{start},#{length}")
    ...
    @Select("select name from info_line where id = #{LineId}")
    String queryLineNameByLineId(String LineId);
