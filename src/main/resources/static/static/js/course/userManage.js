...
function initSchoolUserTable() {
	...
	schoolUserTable = $('#schoolUserTable').DataTable({
		// url
		"sAjaxSource" : "sys/queryUsersList", 
	...
	$.ajax({
		...
		success:function(data) {
		   ...
		   if(data.status=="success") {
			   ...
			   var lineArr = usersData.lineName.split(",");
			   console.log(lineArr);
			   $("input[type='checkbox']").each(function(){
				   console.log(lineArr.indexOf($(this).val()));
				   if(lineArr.indexOf($(this).val())!="-1"){
...
function resetPassword(userId){
	showConfirmModal("是否为该用户重置密码！",function(){
		...
		$.ajax({
			...
			error:function(e) {
			    showSuccessOrErrorModal("重置密码","error"); 
			}
		});
	});
...
$(document).ready(function(){
	...
	$("#userForm").html5Validate(function() {
		...
		$("input[type='checkbox']:checked").each(function(){
			lineStr.push($(this).val())
		});
		if(lineStr.length == 0){
			 showSuccessOrErrorModal("线路不能为空","error");	
			 return;
		}
